...
//        daySymbolText = self.userDaySymbol ?? Calendar.current.shortStandaloneWeekdaySymbols
    ...
    var averageWidth: CGFloat {
        //  - 20
        return (collectionView.frame.width - widthOfTimeAxis) / CGFloat(daySymbols.count) - 5
    }
...
//        collectionView.scrollToItem(at: IndexPath(row: 0, section: 0), at: .top, animated: false)
        ...
        for (index, courseItem) in courseItems.enumerated() {
            ...
            // cell 시작 포인트
            let position_x = collectionView.bounds.minX + widthOfTimeAxis + (averageWidth + 5) * CGFloat(weekdayIndex) + rectEdgeInsets.left + 5
            // 오토레이아웃 30 / 6
...
//            print("Bounds minY: \(collectionView.bounds.minY)")
            ...
            let width = averageWidth + 5 // 오토레이아웃 30 / 6
...
//        print("@Log CourseItem - \(courseItems)")
//        print("@Log senderView - \(sender.view?.tag ?? nil)")
//        print("@Log - \(sender)")
        if let view = sender.view {
            let course = courseItems[view.tag]
